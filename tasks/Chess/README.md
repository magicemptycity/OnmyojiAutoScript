...
| 按剩余人数提前退出 | 每个回目开始时读取右侧存活人数，达到阈值后主动退出；默认关闭 |
| 提前退出人数阈值 | 存活人数小于或等于该值时触发；`0` 表示不启用 |
| 执行次数 | 完成指定局数后结束；`-1` 表示无限循环 |
| 运行时间限制 | 到达限制后不再开启下一局，当前对局仍正常完成 |
| 匹配限制时间 | 单次匹配超时后重试；连续三次超时后结束任务 |
| 刷满鼬乐币 | 识别到 `600/600` 时结束任务 |
| 刷新非前40%符咒 | 保留同品质收益排名前40%的符咒，刷新其余选项 |
...
1. 读取阵容、执行次数、运行限制、鼬乐币、保段位和可选提前退出配置。
