...
class MultiAccountKekkaiActivationNewAccount(ConfigBase, extra="allow"):
    ...
    # 仅控制“多账号多任务挂卡新”内的该账号，和下方 Scheduler.Enable 分离。
    enabled: bool = Field(
        default=True,
        title="启用该功能账号",
        description="停用后仅挂卡新跳过该账号，保留 Scheduler、禁止挂卡时段和私有配置。",
    )
    ...
    # 私有模式使用账号自己的配置；公共模式读取当前 OAS 实例挂卡配置。
