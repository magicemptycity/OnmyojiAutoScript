...
class ScriptTask(MultiAccountRepeatNewBase):
    ...
    fade_conf: MultiAccountKekkaiActivationNew = None
    task_display_names: ClassVar[dict[str, str]] = {
        "MultiAccountKekkaiActivationNew": "多账号多任务挂卡新",
    }
    ...
    def run(self) -> None:
        ...
        if self._defer_for_server_update(now):
            ...
            self._publish_activation_overview(None)
        ...
        for index, account in pending:
            ...
            self._publish_activation_overview({"account_index": index + 1})
            ...
            if not self._run_activation_with_retry(account):
                overall_failed = True
            self._refresh_outer_next_run()
            self._publish_activation_overview(None)
        ...
        self._publish_activation_overview(None)
        if overall_failed:
            logger.warning("多账号多任务挂卡新本轮存在失败账号，已分别安排重试")
        raise TaskEnd(self.task_name)

    def _collect_pending_accounts(
        self,
        now: datetime,
    ) -> list[tuple[int, MultiAccountKekkaiActivationNewAccount]]:
        """筛选到期账号，并在切号前处理公共/私有禁止挂卡时段。"""
        pending: list[tuple[int, MultiAccountKekkaiActivationNewAccount]] = []
    ...
    def _run_activation_with_retry(self, account: MultiAccountKekkaiActivationNewAccount) -> bool:
        """执行原结界挂卡逻辑；失败重启并最多重试三次。"""
        last_exception: Exception | None = None
        for attempt in range(1, self.task_retry_limit + 1):
            backup = self._apply_account_activation_config(account)
            ...
            try:
                ...
                def capture(
                    ...
                    **kwargs,
                    ...
                    if task is not None and convert_to_underscore(str(task)) != "kekkai_activation":
                        return
                ...
                # 内层仅负责计算本账号下次挂卡时间，不能写入公共结界挂卡 scheduler。
                ...
                # 挂卡逻辑会更新“未发现卡次数”；私有模式必须写回当前账号，
                # 否则恢复公共配置时会丢失该账号的连续未命中状态。
                if getattr(account.config_mode, "value", account.config_mode) != "public":
                    account.private_config = {
                        "activation_config": self.config.kekkai_activation.activation_config.model_dump()
                    }
                self._restore_account_activation_config(backup)
        ...
        self._schedule_account_next_run(account, success=False, start_time=datetime.now())
        self._push_error_notification(account, "结界挂卡", last_exception)
        logger.warning(
            "%s-%s 挂卡连续失败，安排在 %s 重试",
            account.character,
            account.svr,
    ...
    def _apply_account_activation_config(self, account: MultiAccountKekkaiActivationNewAccount):
        """临时套用账号独立挂卡配置，并由 Pydantic 统一转换存储格式。"""
        backup = copy.deepcopy(self.config.kekkai_activation.activation_config)
        if getattr(account.config_mode, "value", account.config_mode) == "public":
            return backup
        overrides = account.private_config.get("activation_config", {})
        if not isinstance(overrides, dict):
            raise ValueError("账号私有挂卡配置无效：activation_config 必须是对象")
        ...
        try:
            active = model_with_field_overrides(
                ActivationConfig(),
                overrides,
                # 兼容历史配置中已从当前挂卡模型移除的字段。
                ignore_unknown=True,
            )
        except (ValidationError, ValueError) as exc:
            raise ValueError(f"账号私有挂卡配置无效：{exc}") from exc
        self.config.kekkai_activation.activation_config = active
        return backup

    def _restore_account_activation_config(self, backup: ActivationConfig) -> None:
        self.config.kekkai_activation.activation_config = backup
    ...
    def _defer_for_server_update(self, now: datetime) -> bool:
        ...
        if not delayed:
            return False
        logger.warning("服务器维护期间，已将 %s 个账号的挂卡时间顺延到 %s", delayed, target)
        self._save_repeat_config()
        return True
    ...
    def _set_account_next_run(
        self, account: MultiAccountKekkaiActivationNewAccount, next_run: datetime
    ) -> None:
        next_run = next_run.replace(microsecond=0)
        account.scheduler.next_run = next_run
        account.next_activation_time = next_run
    ...
    def _schedule_account_next_run(
        self, account: MultiAccountKekkaiActivationNewAccount, *, success: bool, start_time: datetime
    ) -> None:
    ...
    def _publish_activation_overview(self, active: dict[str, int] | None) -> None:
        state_queue = getattr(self, "state_queue", None)
        if state_queue is not None:
            state_queue.put({
                "multi_account_overview": {"kind": "activation", "active": active}
            })
